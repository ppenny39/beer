...
# OpenAI API 키 설정
openai.api_key = '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'
...
def main():
    ...
    # 사용자 입력값 받기
    알코올도수 = st.selectbox("알코올(무알코올 맥주와 일반 맥주중 선택해 주세요)", ["있음", "없음"])
    맥아 = st.selectbox("맥아(맥아의 종류를 선택해주세요. 가장 대중적인 것은 보리 맥아 입니다.)", ["보리", "밀", "라이"])
    종류 = st.selectbox("종류(맥주의 종류를 선택해 주세요.)", ["에일", "라거", "람빅", "크래프트"])
    칼로리 = st.selectbox("칼로리(칼로리 수준을 선택해주세요)", ["낮음", "보통", "높음"])
    맛 = st.selectbox("맛(원하시는 맛을 선택해주세요.)", ["쓴맛", "깔끔한 맛", "고소한 맛", "상쾌한 맛", "고급스러운 맛", "풍미로운 맛", "시원한 맛"])
    생산지 = st.selectbox("생산지(원하시는 생산지를 선택해주세요)", ["한국", "미국", "독일", "폴란드", "벨기에", "일본", "네덜란드"])
